...
import {
  ...
  updateFormSchema,
  startLiveSchema,
} from "@workspace/services/form/schemas/input"
import {
  formOutputSchema,
  formWithFieldsOutputSchema,
} from "@workspace/services/form/schemas/output"
  ...
  getPublic: publicProcedure
    .meta({
      openapi: {
        ...
        path: "/forms/explore",
        tags: TAGS,
        summary: "List public forms for explore page",
      },
    })
    .input(z.undefined())
    .output(z.array(formOutputSchema))
  ...
  clone: protectedProcedure
    .meta({
      openapi: {
        ...
        path: "/forms/{id}/clone",
        tags: TAGS,
        summary: "Clone a form (copies all fields, resets to draft)",
      },
    })
    .input(z.object({ id: z.string().uuid() }))
    .output(formOutputSchema)
  ...
  startLive: protectedProcedure
    .meta({
      openapi: {
        ...
        path: "/forms/{id}/live",
        tags: TAGS,
        summary: "Start a live session for a form",
      },
    })
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      if (!existing.published)
        throw new TRPCError({
          code: "BAD_REQUEST",
          message: "Publish the form before going live",
        })
